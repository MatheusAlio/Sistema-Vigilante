...
class ConfiguracoesGerais(tk.Toplevel):
    def __init__(self, parent):
        ...
        # Cabeçalho
        tk.Label(self, text="Configurações de Arduinos e Pânicos",
    ...
    def criar_bloco_panico_geral(self):
        ...
        for i in range(1, 9):
            ...
            tk.Label(linha, text=f"IP Pânico {i}:", bg="#2C2F44", fg="white").pack(side="left", padx=5)
    ...
    def testar_conexoes(self):
        ...
        for i in range(1, 9):
            ...
            try:
                ...
                resultados.append(f"Pânico {i}: {'✅ OK' if r.status_code == 200 else f'⚠️ Erro {r.status_code}'}")
            except Exception:
...
def enviar_panico_geral(self):
    """Ativa todos os pânicos gerais conforme configurado no config.json"""
    try:
        ...
        for idx, ativo in enumerate(portas):
            if ativo:
                try:
                    ...
                    resultados.append(f"Pânico {i} Porta {idx+1}: ✅ Enviado")
                except Exception as e:
                    resultados.append(f"Pânico {i} Porta {idx+1}: ❌ Erro ({e})")

    messagebox.showinfo("Pânico Geral", "\n".join(resultados))
    ...
    def salvar_tudo(self):
        ...
        # Inclui dados do bloco Pânico Geral
        for i in range(1, 9):
...
class TelaControlePortoes:
    def __init__(self, master):
        ...
        self.urls_cameras = {i: f"rtsp://admin:condo.54@192.168.150.{148+i}:554/" for i in range(1,9)}
    ...
    def acionar_panico(self):
        if not self.panico_ativo:
            ...
            self.btn_panico.config(text="Cancelar Pânico",bg="#8B0000")
            self.status_bar.config(text="⚠️ PÂNICO ATIVADO!",fg="yellow")
            for ip in self.arduino_ips.values():
                try: requests.get(f"http://{ip}/comando?portao=2",timeout=5)
                except: pass
            messagebox.showwarning("Pânico","🚨 PÂNICO ATIVADO!")
        else:
            self.panico_ativo = False
            self.btn_panico.config(text="Pânico Geral",bg="#FF0000")
            self.status_bar.config(text=f"IP do computador: {pegar_ip_local()}",fg="white")
            messagebox.showinfo("Pânico","✅ Estado de pânico cancelado.")
